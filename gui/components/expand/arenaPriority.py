...
from qfluentwidgets import LineEdit, ComboBox, CheckBox
...
class Layout(QWidget):
    def __init__(self, parent=None, config=None):
        ...
        self.input_1.editingFinished.connect(self.__accept_1)
        self.input_2.editingFinished.connect(self.__accept_2)
        self.input_3.currentIndexChanged.connect(self.__accept_3)
    ...
    def __accept_1(self):
        text = self.input_1.text().strip()
        if text == '' or text == '-':
            return
        self.level_diff = int(text)
        self.config.set('ArenaLevelDiff', self.level_diff)

    def __accept_2(self, _=None):
        text = self.input_2.text().strip()
        if text == '':
            return
        self.refresh_times = int(text)
