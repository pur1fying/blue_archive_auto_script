...
import uuid
import re
from .preprocessing_name import preprocessing_name
...
def device_config(simulator_type, multi_instance, latest_adb_port = None,latest_command_line = None):
    # 从json文件中读取字典
    multi_instance = preprocessing_name(simulator_type, multi_instance)

    global data
    if os.path.exists('./config/device.json'):
    ...
    for id, value in data.items():
        if value['simulator_type'] == simulator_type and value['multi_instance'] == multi_instance:
            return id
        elif value['latest_adb_port'] == latest_adb_port:
            return id
    ...
    # 注：此处未作合法性校验
    if latest_adb_port == None:
        latest_adb_port = get_simulator_port(simulator_type, multi_instance)

    # 将UUID和对应的输入存储到字典中
    data[id] = {
        'simulator_type': simulator_type,
        'multi_instance': multi_instance,
        'latest_adb_port': latest_adb_port,
...
def load_data(id):
    ...
    try:
        ...
        if device_info is not None:
            ...
            if re.match(ipv4_pattern, latest_adb_port) or re.match(ipv6_pattern, latest_adb_port):
                # 将最新的adb端口写入到字典中
                device_info['latest_adb_port'] = latest_adb_port
