...
### 文字识别核心用途
**BAAS**主要使用文字识别来识别:
1. 数值: 自动战斗中BOSS的血量, 倒计时信息, 制造中投入物资的数量, 活动图中的关卡ID等
2. 文本: 咖啡厅邀请券中角色的名称, 自动删除好友时识别ID

### 多语言识别能力 
**BAAS_OCR**支持[多种语言](#可选的语言)

### 低延迟
自动战斗需要高频次地获取界面状态，例如在战斗循环中以每秒数次的频率读取血量数值、倒计时等信息。**BAAS_OCR**对短文本可以实现毫秒级的识别速度，

### 跨平台
**BAAS_OCR**同时支持 Windows / Linux / macOS / Android。
...
3. 可使用的预编译版本
    -   | 操作系统    | 架构        | 操作系统版本       | 可执行程序分支                                                                                    |
        ...
        | Android | arm64-v8a | 最新版本         | [android-arm64-v8a](https://github.com/pur1fying/BAAS_Cpp_prebuild/tree/android-arm64-v8a) |
        | Android | x86_84    | 最新版本         | [android-x86_64](https://github.com/pur1fying/BAAS_Cpp_prebuild/tree/android-x86_64)       |
...
6. 有关Android: 
    1. Android系统并不支持共享内存, 因此在Android系统上, 图像只能通过传递图像png编码后的二进制数据或者提供图像路径的方式传递给ocr_server
    2. Android预编译包中并不包含可执行程序, 通过so库中的函数启动服务
