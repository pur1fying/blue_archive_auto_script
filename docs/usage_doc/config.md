...
1. 推图关卡中填写的字符串不应该超出这些字符或单词 `"-" , "sss", "present", "task", "," , 和数字`
2. 每一条配置由```","```隔开
3. 每一条配置填写说明:
   1. 字符串没有关键字"sss","present","task"
      ...
      | 填写       | BAAS                                                   |
      |----------|--------------------------------------------------------|
      | 15, 12-2 | 根据困难图推图设置中三个的开关 将(15-1,15-2,15-3,12-2)打到sss/拿礼物/完成挑战任务 |
   2. 一个数字并跟字符串，且用"-"分隔
      例:
   
      | 填写             | BAAS                       |
      |----------------|----------------------------|
      | 15-sss-present | 将(15-1,15-2,15-3)打到sss并拿礼物 |
   3. 两个数字( 指定到对应关卡 )
      例:
   
      | 填写            | BAAS               |
      |---------------|--------------------|
      | 15-3-sss-task | 将15-3打到sss并且完成挑战任务 |
    4. 例: **开关都开启**
   
       | 填写               | BAAS                                                           |
       |------------------|----------------------------------------------------------------|
       | 7,8-sss,9-3-task | 依次执行(7-1,7-2,7-3)打到sss,拿礼物并完成挑战任务,(8-1,8-2,8-3)打到sss，9-3完成挑战任务 |
   
**note**: 
1. 一些困难图需要三支队伍, 第三支队伍的属性均与该区域所需属性的第一个属性相同
2. **BAAS**会自动判断关卡是否已经打到sss，是否拿了礼物，如果已经打到sss或拿了礼物，则跳过该关卡。
