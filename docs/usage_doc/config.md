...
3. 保存设置时会校验学生别名、当前服务器实装状态和模型支持状态。
4. 国服、国际服和日服共用学生身份识别模型。
5. 只有粉框、当前可以获得好感的学生会触发指定学生优先；灰框学生不会触发点击。
6. 模型缺失、损坏或识别结果未达到置信度阈值时，脚本会忽略身份识别结果，并按普通日程规则继续执行。
...
:::warning
历史头像只作为模型的种子数据。设置界面提示“模型尚未验证”的学生不会触发指定学生优先，必须补充新版 UI 的真实标注数据并通过验证后才能加入支持列表。
:::
