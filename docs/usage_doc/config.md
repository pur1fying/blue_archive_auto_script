...
6. [**小春版配队指南**](#小春版配队指南)
7. **BAAS会**根据**普通图推图设置**中[队伍属性配置](/develop_doc/script/config#burst1-burst2-pierce1-pierce2-mystic1-mystic2-shock1-shock2)和[选队逻辑](#选队逻辑)寻找合适配置进行推图

#### 选队逻辑:
1. 当选队逻辑选项为 `"按预设选队"`时: 
    - **BAAS**会根据需要属性的预设队伍进行选队, 你需要设置**爆发, 贯穿, 神秘, 振动**四种属性在预设的行列号
    - 你需要人为保证所选择的队伍不会有角色冲突
    - 例:
      1.设四种属性(爆发, 贯穿, 神秘, 振动)分别在第一行第一列, 第一行第二列, 第二行第一列, 第二行第二列
    
      | 需要的属性     | 选择的队伍      |
      |-----------|------------|
      | a. 爆发, 神秘 | 一行一列, 二行一列 |
      | b. 爆发, 爆发 | 一行一列, 二行一列 | 
        b.解释 : 由于只设置一支爆发预设, 第二支队伍无爆发队伍可选, 按照克制顺序选择**贯穿**队伍
      
#### 小春版配队指南
将选队逻辑选项选择`"按顺序选队"`, 下面的数字编号为选取队伍的顺序(必须由小到大)
假设数字编号为 [1, 2, 3]
则无论关卡属性如何, 都会按照1, 2, 3的顺序选取队伍
