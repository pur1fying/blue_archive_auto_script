...
## 一些关键的设置
::: info
    如果你是Steam / 日服PC端用户, 请参阅左侧PC端用户配置
:::
...
#### 如何设置BAAS优先执行指定学生所在的日程

1. 开启选项 `优先做指定角色存在的日程`。
2. 在 `指定角色` 中填写学生的英文配置名；多个学生使用 `>` 分隔，越靠前优先级越高。
3. 保存设置时会校验学生名称；运行时如果身份图库没有对应原型，会记录警告并按普通日程规则继续执行。
4. 国服、国际服和日服共用学生身份识别模型。
5. 只有被定位器判定为粉框、当前可以获得好感的学生会触发指定学生优先；被判定为灰框的头像不会触发点击。当前模型仍可能把少量真实灰框误判为粉框，已知位置记录在独立测试报告中。
6. 有效头像裁图采用全局 Top-1 结果，相似度和候选分差只用于日志诊断，不作为点击门槛。模型缺失、损坏或裁图无效时，脚本会忽略身份识别结果，并按普通日程规则继续执行。
7. 如果没有收录你想要的角色的模板, 你需要向开发者提供以下截图
    1. 模拟器1280x720分辨率
    2. 图像是无损的(png)
    3. 日程界面中有指定角色
    4. 等待开发者适配
...
:::warning
训练数据来源和验证状态只用于诊断，不会阻止有效的全局 Top-1 结果参与选择。没有真实日程截图的学生仍缺少独立点击证据；当前独立测试还记录了少量真实灰框被误判为粉框的风险。
:::
