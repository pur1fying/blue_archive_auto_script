...
#### 如何设置BAAS做指定角色日程
1. 开启选项```优先做指定角色存在的日程```
2. 按优先顺序填写学生名称；国服、国际服和日服共用270人三语名册
3. 第一次使用时会从公开GitHub Release下载身份模型并校验SHA-256，缓存到```config/cache/student_recognition/<版本>/```
4. 新版“全部日程”弹窗不再滚动。脚本按固定布局读取1至9个非空卡片，并用MobileNetV3全局Top-1识别头像
5. 只有被边框规则判定为粉框、所在卡片仍可执行且Top-1命中目标时才会选择；灰框和已完成卡片不会主动选择
6. 身份分数和候选分差只写入诊断信息，不作为点击阈值。模型缺失、下载失败、损坏或裁图无效时会回退普通日程规则

:::warning
固定边框规则避免了额外检测模型，并在现有冻结回归集的247个可用状态头像上正确区分粉框与灰框；但游戏UI变化后仍可能失效，建议首次使用新版本时观察运行日志。
:::
