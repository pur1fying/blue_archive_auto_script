...
- **分辨率**: 
    1. 推荐使用`窗口模式`, 屏幕比例**必须**选择`16:9`, 也就是启用以下选项
        ![screen_windowed_ratio_16_9.png](/assets/pc_platform/setting_screen_windowed_ratio_16_9.png)
        - 调整为`窗口模式后`, 默认分辨率为1280x720, 也就是**最佳分辨率**, 但是如果通过拉动窗口边框来调整窗口大小, 可能会导致窗口分辨率不是标准的`16:9`, 但是接近`16:9`, 此时**BAAS**不会报错, 但是有**极大的可能**导致无法识别游戏画面
            ![screen_windowed_ratio_16_9.png](/assets/pc_platform/BAAS_log_screen_ratio_not_16_9.png)
