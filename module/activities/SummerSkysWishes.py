...
def to_mission_task_info(self, number):
    ...
    if number in [5, 6, 7, 8]:
        self.swipe(916, 588, 916, 113, duration=0.5, post_sleep_time=0.7)
    if number in [9, 10, 11, 12]:
        self.swipe(943, 588, 943, 0, duration=0.1, post_sleep_time=0.7)
        self.swipe(943, 588, 943, 0, duration=0.1, post_sleep_time=0.7)
...
def exchange_reward(self):
    ...
    while 1:
        while color.is_rgb_in_range(self, 314, 684, 235, 255, 223, 243, 65, 85):
            ...
            to_exchange(self, True)
        if color.is_rgb_in_range(self, 45, 684, 185, 205, 185, 205, 185, 205):
...
def jointTask(self):
    ...
    for i in range(0, tickets[0]):
        ...
        main_story.set_acc_and_auto(self)
        toJointTask(self)
