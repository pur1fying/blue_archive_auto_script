...
def explore_story(self):
    ...
    for i in range(0, 15):
        ...
        if mission == "ALL MISSION SWEEP AVAILABLE":
            ...
            return True
        if mission == 'SUB':
            start_choose_side_team(self, self.config[stage_data['SUB']])
            rgb_possibles = {
                "formation_edit" + str(self.config[stage_data['SUB']]): (1171, 670),
            }
...
def to_story_task_info(self):
    img_possibles = {'activity_menu': (1124, 184)}
