...
def to_story_task_info(self, number):
    lo = [0, 193, 294, 391, 491, 567, 341, 441, 541]
    if number >= 6:
...
def to_mission_task_info(self, number):
    ...
    if number in [5, 6, 7, 8]:
        self.u2_swipe(916, 585, 916, 105, duration=1, post_sleep_time=0.7)
    if number in [9, 10, 11, 12]:
        self.u2_swipe(943, 585, 943, 0, duration=0.1, post_sleep_time=0.7)
        self.u2_swipe(943, 585, 943, 0, duration=0.1, post_sleep_time=0.7)
    img_possibles = {'activity_menu': (1124, lo[index[number - 1]])}
    img_ends = ["normal_task_task-info", "activity_task-info"]
