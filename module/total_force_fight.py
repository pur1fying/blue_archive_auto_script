...
def find_button_y(self, y, temp_ocr):
    log.d("START FIND BUTTON FOR LEVEL " + self.total_force_fight_difficulty_name_ordered[y], 1, logger_box=self.loggerBox)
    ...
    log.d("CAN'T DETECT BRIGHT BUTTON FOR LEVEL " + self.total_force_fight_difficulty_name_ordered[y], 3, logger_box=self.loggerBox)
    return False
...
def fight_difficulty_x(self, x,temp_ocr):
    ...
    log.d("start total force fight difficulty : " + self.total_force_fight_difficulty_name_ordered[x], 1, logger_box=self.loggerBox)
        ...
        for i in range(0, len(self.total_force_fight_difficulty_name)):
            ...
            for j in range(0, len(ocr_res)):
                if kmp(self.total_force_fight_difficulty_name[i].lower(), ocr_res[j]["text"].lower()):
                    ...
                    log.d("detect " + self.total_force_fight_difficulty_name[i] + " " + name[temp] + " button", 1, logger_box=self.loggerBox)
