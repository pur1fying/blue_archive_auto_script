...
def implement(self):
    ...
    while 1:
        if color.is_rgb_in_range(self, 1148, 691, 239, 255, 228, 248, 64, 84) and \
            color.is_rgb_in_range(self, 1142, 649, 239, 255, 228, 248, 64, 84):
            self.logger.info("claim reward")
            self.click(1145, 670,  duration=2, wait_over=True)
            self.click(254, 72,  wait_over=True)
            self.click(254, 72,  wait_over=True)
            to_tasks(self)
        elif color.is_rgb_in_range(self, 1148, 691, 185, 227, 184, 227, 188, 228) and \
            color.is_rgb_in_range(self, 1142, 649, 185, 227, 184, 227, 188, 228):
            self.logger.info("claim all grey")
            break
        else:
            self.logger.info("Can't detect button")
            return True
    if color.is_rgb_in_range(self, 971, 689, 239, 255, 228, 248, 40, 84) and \
        color.is_rgb_in_range(self, 964, 649, 239, 255, 228, 248, 40, 84):
        self.click(976, 670,  duration=0.3, wait_over=True)
        self.click(254, 72,  wait_over=True)
        self.click(254, 72,  wait_over=True)
        to_tasks(self)
    elif color.is_rgb_in_range(self, 959, 694, 210, 230, 210, 230, 210, 230) and \
        color.is_rgb_in_range(self, 957, 650, 210, 230, 210, 230, 210, 230):
        self.logger.info("claim daily pyroxenes grey")
    elif color.is_rgb_in_range(self, 959, 694, 112, 152, 116, 156, 119, 159) and \
        color.is_rgb_in_range(self, 957, 650, 112, 152, 116, 156, 119, 159):
